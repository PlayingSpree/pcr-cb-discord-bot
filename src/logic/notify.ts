...
export async function start(interaction: CommandInteraction) {
    const reply = await interaction.reply({ content: 'กด React ที่หมายเลขเพื่อรับการแจ้งเตือน เมื่อมีการเรียกตีบอสนั้น ๆ', fetchReply: true }) as Message;

    notifyStateData.set(interaction.guildId!, new NotifyState(reply.id));
